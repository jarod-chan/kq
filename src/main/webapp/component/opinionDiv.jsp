...
<div class="chk_opinions" >
	<div>
		<c:forEach var="opinion" items="${opinions}" varStatus="status">
			<div>
				${status.count}.
				<span style="font-weight: bold">${opinion.userName}</span>执行任务<span style="font-weight: bold">${opinion.taskName}</span>【${opinion.result.name}】<c:if test="${not empty opinion.description}">:${opinion.description}</c:if>
			</div>
		</c:forEach>
	</div>
</div>
